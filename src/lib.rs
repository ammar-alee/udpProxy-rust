...
impl UdpSocket {
    ...
    /// # }
    /// ```
    pub fn bind<Addrs: ToSocketAddrs>(addrs: Addrs) -> Result<UdpSocket, std::io::Error> {
...
fn each_addr<A: ToSocketAddrs, F, T>(addr: A, mut f: F) -> std::io::Result<T>
    where F: FnMut(&SocketAddr) -> std::io::Result<T>
{
